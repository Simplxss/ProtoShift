package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarExpeditionAllDataReq, type = 1)
    public class HandlerAvatarExpeditionAllDataReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarExpeditionAllDataReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarExpeditionAllDataReqOuterClass.AvatarExpeditionAllDataReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarExpeditionAllDataReqOuterClass.AvatarExpeditionAllDataReq.parseFrom(payload)
                            ), q);
