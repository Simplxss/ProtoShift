package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtDestroyGadgetNotify, type = 1)
    public class HandlerEvtDestroyGadgetNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtDestroyGadgetNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtDestroyGadgetNotifyOuterClass.EvtDestroyGadgetNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtDestroyGadgetNotifyOuterClass.EvtDestroyGadgetNotify.parseFrom(payload)
                            ), q);
