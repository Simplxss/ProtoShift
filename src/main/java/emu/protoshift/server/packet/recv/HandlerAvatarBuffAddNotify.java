package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarBuffAddNotify, type = 1)
    public class HandlerAvatarBuffAddNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarBuffAddNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarBuffAddNotifyOuterClass.AvatarBuffAddNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarBuffAddNotifyOuterClass.AvatarBuffAddNotify.parseFrom(payload)
                            ), q);
