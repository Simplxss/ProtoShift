package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.DungeonPlayerDieRsp, type = 1)
    public class HandlerDungeonPlayerDieRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.DungeonPlayerDieRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.DungeonPlayerDieRspOuterClass.DungeonPlayerDieRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.DungeonPlayerDieRspOuterClass.DungeonPlayerDieRsp.parseFrom(payload)
                            ), q);
