package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetUpAvatarTeamReq, type = 1)
    public class HandlerSetUpAvatarTeamReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetUpAvatarTeamReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetUpAvatarTeamReqOuterClass.SetUpAvatarTeamReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetUpAvatarTeamReqOuterClass.SetUpAvatarTeamReq.parseFrom(payload)
                            ), q);
