package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EnterSceneReadyReq, type = 1)
    public class HandlerEnterSceneReadyReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterSceneReadyReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterSceneReadyReqOuterClass.EnterSceneReadyReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterSceneReadyReqOuterClass.EnterSceneReadyReq.parseFrom(payload)
                            ), q);
