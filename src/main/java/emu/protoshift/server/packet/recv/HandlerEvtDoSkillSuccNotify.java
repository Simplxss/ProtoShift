package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtDoSkillSuccNotify, type = 1)
    public class HandlerEvtDoSkillSuccNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtDoSkillSuccNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtDoSkillSuccNotifyOuterClass.EvtDoSkillSuccNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtDoSkillSuccNotifyOuterClass.EvtDoSkillSuccNotify.parseFrom(payload)
                            ), q);
