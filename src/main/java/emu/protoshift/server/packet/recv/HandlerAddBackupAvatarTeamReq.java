package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AddBackupAvatarTeamReq, type = 1)
    public class HandlerAddBackupAvatarTeamReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AddBackupAvatarTeamReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AddBackupAvatarTeamReqOuterClass.AddBackupAvatarTeamReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AddBackupAvatarTeamReqOuterClass.AddBackupAvatarTeamReq.parseFrom(payload)
                            ), q);
