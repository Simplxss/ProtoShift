    ...
    public class HandlerWidgetSlotChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WidgetSlotChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WidgetSlotChangeNotifyOuterClass.WidgetSlotChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WidgetSlotChangeNotifyOuterClass.WidgetSlotChangeNotify.parseFrom(payload)
                            ), q);
