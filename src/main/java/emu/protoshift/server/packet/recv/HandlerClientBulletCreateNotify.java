package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ClientBulletCreateNotify, type = 1)
    public class HandlerClientBulletCreateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ClientBulletCreateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ClientBulletCreateNotifyOuterClass.ClientBulletCreateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ClientBulletCreateNotifyOuterClass.ClientBulletCreateNotify.parseFrom(payload)
                            ), q);
