package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerEnterSceneInfoNotify, type = 1)
    public class HandlerPlayerEnterSceneInfoNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerEnterSceneInfoNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerEnterSceneInfoNotifyOuterClass.PlayerEnterSceneInfoNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerEnterSceneInfoNotifyOuterClass.PlayerEnterSceneInfoNotify.parseFrom(payload)
                            ), q);
