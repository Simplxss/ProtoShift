    ...
    public class HandlerWorldPlayerReviveReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WorldPlayerReviveReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WorldPlayerReviveReqOuterClass.WorldPlayerReviveReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WorldPlayerReviveReqOuterClass.WorldPlayerReviveReq.parseFrom(payload)
                            ), q);
