    ...
    public class HandlerSceneDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneDataNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneDataNotifyOuterClass.SceneDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneDataNotifyOuterClass.SceneDataNotify.parseFrom(payload)
                            ), q);
