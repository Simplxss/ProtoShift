package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarWearFlycloakRsp, type = 1)
    public class HandlerAvatarWearFlycloakRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarWearFlycloakRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarWearFlycloakRspOuterClass.AvatarWearFlycloakRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarWearFlycloakRspOuterClass.AvatarWearFlycloakRsp.parseFrom(payload)
                            ), q);
