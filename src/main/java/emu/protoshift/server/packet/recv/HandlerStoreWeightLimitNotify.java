package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.StoreWeightLimitNotify, type = 1)
    public class HandlerStoreWeightLimitNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.StoreWeightLimitNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.StoreWeightLimitNotifyOuterClass.StoreWeightLimitNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.StoreWeightLimitNotifyOuterClass.StoreWeightLimitNotify.parseFrom(payload)
                            ), q);
