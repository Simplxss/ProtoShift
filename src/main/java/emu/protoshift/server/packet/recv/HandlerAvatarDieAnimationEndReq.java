package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarDieAnimationEndReq, type = 1)
    public class HandlerAvatarDieAnimationEndReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarDieAnimationEndReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarDieAnimationEndReqOuterClass.AvatarDieAnimationEndReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarDieAnimationEndReqOuterClass.AvatarDieAnimationEndReq.parseFrom(payload)
                            ), q);
