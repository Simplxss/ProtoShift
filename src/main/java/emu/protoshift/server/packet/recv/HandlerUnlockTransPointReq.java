package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.UnlockTransPointReq, type = 1)
    public class HandlerUnlockTransPointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.UnlockTransPointReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.UnlockTransPointReqOuterClass.UnlockTransPointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.UnlockTransPointReqOuterClass.UnlockTransPointReq.parseFrom(payload)
                            ), q);
