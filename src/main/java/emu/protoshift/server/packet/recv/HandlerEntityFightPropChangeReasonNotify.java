package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EntityFightPropChangeReasonNotify, type = 1)
    public class HandlerEntityFightPropChangeReasonNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EntityFightPropChangeReasonNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EntityFightPropChangeReasonNotifyOuterClass.EntityFightPropChangeReasonNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EntityFightPropChangeReasonNotifyOuterClass.EntityFightPropChangeReasonNotify.parseFrom(payload)
                            ), q);
