package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtCostStaminaNotify, type = 1)
    public class HandlerEvtCostStaminaNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtCostStaminaNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtCostStaminaNotifyOuterClass.EvtCostStaminaNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtCostStaminaNotifyOuterClass.EvtCostStaminaNotify.parseFrom(payload)
                            ), q);
