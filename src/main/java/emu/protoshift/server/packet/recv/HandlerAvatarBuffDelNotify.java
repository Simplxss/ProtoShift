package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarBuffDelNotify, type = 1)
    public class HandlerAvatarBuffDelNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarBuffDelNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarBuffDelNotifyOuterClass.AvatarBuffDelNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarBuffDelNotifyOuterClass.AvatarBuffDelNotify.parseFrom(payload)
                            ), q);
