package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetScenePointReq, type = 1)
    public class HandlerGetScenePointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetScenePointReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetScenePointReqOuterClass.GetScenePointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetScenePointReqOuterClass.GetScenePointReq.parseFrom(payload)
                            ), q);
