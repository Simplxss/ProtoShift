    ...
    public class HandlerSceneTransToPointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneTransToPointReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneTransToPointReqOuterClass.SceneTransToPointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneTransToPointReqOuterClass.SceneTransToPointReq.parseFrom(payload)
                            ), q);
