package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.LeaveSceneReq, type = 1)
    public class HandlerLeaveSceneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.LeaveSceneReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.LeaveSceneReqOuterClass.LeaveSceneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.LeaveSceneReqOuterClass.LeaveSceneReq.parseFrom(payload)
                            ), q);
