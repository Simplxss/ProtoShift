package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.BackMyWorldReq, type = 1)
    public class HandlerBackMyWorldReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.BackMyWorldReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.BackMyWorldReqOuterClass.BackMyWorldReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.BackMyWorldReqOuterClass.BackMyWorldReq.parseFrom(payload)
                            ), q);
