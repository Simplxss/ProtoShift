package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerAskFriendListRsp, type = 1)
    public class HandlerGetPlayerAskFriendListRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerAskFriendListRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerAskFriendListRspOuterClass.GetPlayerAskFriendListRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerAskFriendListRspOuterClass.GetPlayerAskFriendListRsp.parseFrom(payload)
                            ), q);
