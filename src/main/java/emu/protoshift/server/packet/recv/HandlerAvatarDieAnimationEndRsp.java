    ...
    public class HandlerAvatarDieAnimationEndRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarDieAnimationEndRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarDieAnimationEndRspOuterClass.AvatarDieAnimationEndRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarDieAnimationEndRspOuterClass.AvatarDieAnimationEndRsp.parseFrom(payload)
                            ), q);
