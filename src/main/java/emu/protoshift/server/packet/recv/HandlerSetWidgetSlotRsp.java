package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetWidgetSlotRsp, type = 1)
    public class HandlerSetWidgetSlotRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetWidgetSlotRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetWidgetSlotRspOuterClass.SetWidgetSlotRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetWidgetSlotRspOuterClass.SetWidgetSlotRsp.parseFrom(payload)
                            ), q);
