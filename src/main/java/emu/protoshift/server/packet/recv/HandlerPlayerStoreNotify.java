package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerStoreNotify, type = 1)
    public class HandlerPlayerStoreNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerStoreNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerStoreNotifyOuterClass.PlayerStoreNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerStoreNotifyOuterClass.PlayerStoreNotify.parseFrom(payload)
                            ), q);
