package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.LifeStateChangeNotify, type = 1)
    public class HandlerLifeStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.LifeStateChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.LifeStateChangeNotifyOuterClass.LifeStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.LifeStateChangeNotifyOuterClass.LifeStateChangeNotify.parseFrom(payload)
                            ), q);
