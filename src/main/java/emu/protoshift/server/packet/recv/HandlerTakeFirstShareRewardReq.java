package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.TakeFirstShareRewardReq, type = 1)
    public class HandlerTakeFirstShareRewardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.TakeFirstShareRewardReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.TakeFirstShareRewardReqOuterClass.TakeFirstShareRewardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.TakeFirstShareRewardReqOuterClass.TakeFirstShareRewardReq.parseFrom(payload)
                            ), q);
