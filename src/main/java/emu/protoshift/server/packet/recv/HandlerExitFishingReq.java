package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ExitFishingReq, type = 1)
    public class HandlerExitFishingReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ExitFishingReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ExitFishingReqOuterClass.ExitFishingReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ExitFishingReqOuterClass.ExitFishingReq.parseFrom(payload)
                            ), q);
