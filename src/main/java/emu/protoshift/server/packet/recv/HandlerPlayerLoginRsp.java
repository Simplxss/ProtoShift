package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerLoginRsp, type = 1)
    public class HandlerPlayerLoginRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerLoginRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerLoginRspOuterClass.PlayerLoginRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerLoginRspOuterClass.PlayerLoginRsp.parseFrom(payload)
                            ), q);
