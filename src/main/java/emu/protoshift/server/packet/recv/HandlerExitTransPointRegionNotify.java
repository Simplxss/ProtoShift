package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ExitTransPointRegionNotify, type = 1)
    public class HandlerExitTransPointRegionNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ExitTransPointRegionNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ExitTransPointRegionNotifyOuterClass.ExitTransPointRegionNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ExitTransPointRegionNotifyOuterClass.ExitTransPointRegionNotify.parseFrom(payload)
                            ), q);
