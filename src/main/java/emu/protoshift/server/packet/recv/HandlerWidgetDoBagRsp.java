package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.WidgetDoBagRsp, type = 1)
    public class HandlerWidgetDoBagRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WidgetDoBagRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WidgetDoBagRspOuterClass.WidgetDoBagRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WidgetDoBagRspOuterClass.WidgetDoBagRsp.parseFrom(payload)
                            ), q);
