package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.WidgetDoBagReq, type = 1)
    public class HandlerWidgetDoBagReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WidgetDoBagReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WidgetDoBagReqOuterClass.WidgetDoBagReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WidgetDoBagReqOuterClass.WidgetDoBagReq.parseFrom(payload)
                            ), q);
