package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.InvestigationQuestDailyNotify, type = 1)
    public class HandlerInvestigationQuestDailyNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.InvestigationQuestDailyNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.InvestigationQuestDailyNotifyOuterClass.InvestigationQuestDailyNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.InvestigationQuestDailyNotifyOuterClass.InvestigationQuestDailyNotify.parseFrom(payload)
                            ), q);
