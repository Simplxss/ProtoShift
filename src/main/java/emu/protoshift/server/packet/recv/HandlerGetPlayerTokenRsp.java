package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerTokenRsp, type = 1)
    public class HandlerGetPlayerTokenRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerTokenRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerTokenRspOuterClass.GetPlayerTokenRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerTokenRspOuterClass.GetPlayerTokenRsp.parseFrom(payload)
                            ), q);
