package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PullRecentChatReq, type = 1)
    public class HandlerPullRecentChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PullRecentChatReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PullRecentChatReqOuterClass.PullRecentChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PullRecentChatReqOuterClass.PullRecentChatReq.parseFrom(payload)
                            ), q);
