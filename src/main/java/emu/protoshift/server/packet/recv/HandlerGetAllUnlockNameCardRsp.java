package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetAllUnlockNameCardRsp, type = 1)
    public class HandlerGetAllUnlockNameCardRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetAllUnlockNameCardRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetAllUnlockNameCardRspOuterClass.GetAllUnlockNameCardRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetAllUnlockNameCardRspOuterClass.GetAllUnlockNameCardRsp.parseFrom(payload)
                            ), q);
