package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneEntityDisappearNotify, type = 1)
    public class HandlerSceneEntityDisappearNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneEntityDisappearNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneEntityDisappearNotifyOuterClass.SceneEntityDisappearNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneEntityDisappearNotifyOuterClass.SceneEntityDisappearNotify.parseFrom(payload)
                            ), q);
