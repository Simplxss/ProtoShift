package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarLifeStateChangeNotify, type = 1)
    public class HandlerAvatarLifeStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarLifeStateChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarLifeStateChangeNotifyOuterClass.AvatarLifeStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarLifeStateChangeNotifyOuterClass.AvatarLifeStateChangeNotify.parseFrom(payload)
                            ), q);
