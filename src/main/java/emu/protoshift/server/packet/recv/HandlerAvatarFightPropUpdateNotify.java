package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarFightPropUpdateNotify, type = 1)
    public class HandlerAvatarFightPropUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarFightPropUpdateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarFightPropUpdateNotifyOuterClass.AvatarFightPropUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarFightPropUpdateNotifyOuterClass.AvatarFightPropUpdateNotify.parseFrom(payload)
                            ), q);
