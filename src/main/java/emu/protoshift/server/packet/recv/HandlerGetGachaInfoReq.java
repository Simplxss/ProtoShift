package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetGachaInfoReq, type = 1)
    public class HandlerGetGachaInfoReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetGachaInfoReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetGachaInfoReqOuterClass.GetGachaInfoReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetGachaInfoReqOuterClass.GetGachaInfoReq.parseFrom(payload)
                            ), q);
