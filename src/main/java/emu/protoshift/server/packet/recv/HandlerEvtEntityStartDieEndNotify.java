package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtEntityStartDieEndNotify, type = 1)
    public class HandlerEvtEntityStartDieEndNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtEntityStartDieEndNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtEntityStartDieEndNotifyOuterClass.EvtEntityStartDieEndNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtEntityStartDieEndNotifyOuterClass.EvtEntityStartDieEndNotify.parseFrom(payload)
                            ), q);
