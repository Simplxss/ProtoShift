package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EnterTransPointRegionNotify, type = 1)
    public class HandlerEnterTransPointRegionNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterTransPointRegionNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterTransPointRegionNotifyOuterClass.EnterTransPointRegionNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterTransPointRegionNotifyOuterClass.EnterTransPointRegionNotify.parseFrom(payload)
                            ), q);
