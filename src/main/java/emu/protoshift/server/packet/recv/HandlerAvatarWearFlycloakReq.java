package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarWearFlycloakReq, type = 1)
    public class HandlerAvatarWearFlycloakReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarWearFlycloakReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarWearFlycloakReqOuterClass.AvatarWearFlycloakReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarWearFlycloakReqOuterClass.AvatarWearFlycloakReq.parseFrom(payload)
                            ), q);
