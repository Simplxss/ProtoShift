package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ChangeAvatarRsp, type = 1)
    public class HandlerChangeAvatarRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ChangeAvatarRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ChangeAvatarRspOuterClass.ChangeAvatarRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ChangeAvatarRspOuterClass.ChangeAvatarRsp.parseFrom(payload)
                            ), q);
