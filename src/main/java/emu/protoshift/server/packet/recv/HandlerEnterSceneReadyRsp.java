package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EnterSceneReadyRsp, type = 1)
    public class HandlerEnterSceneReadyRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterSceneReadyRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterSceneReadyRspOuterClass.EnterSceneReadyRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterSceneReadyRspOuterClass.EnterSceneReadyRsp.parseFrom(payload)
                            ), q);
