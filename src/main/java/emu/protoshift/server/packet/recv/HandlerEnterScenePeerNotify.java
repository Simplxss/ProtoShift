package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EnterScenePeerNotify, type = 1)
    public class HandlerEnterScenePeerNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterScenePeerNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterScenePeerNotifyOuterClass.EnterScenePeerNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterScenePeerNotifyOuterClass.EnterScenePeerNotify.parseFrom(payload)
                            ), q);
