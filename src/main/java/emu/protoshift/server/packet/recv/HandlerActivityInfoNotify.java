package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ActivityInfoNotify, type = 1)
    public class HandlerActivityInfoNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ActivityInfoNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ActivityInfoNotifyOuterClass.ActivityInfoNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ActivityInfoNotifyOuterClass.ActivityInfoNotify.parseFrom(payload)
                            ), q);
