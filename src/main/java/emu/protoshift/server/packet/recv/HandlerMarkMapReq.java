package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.MarkMapReq, type = 1)
    public class HandlerMarkMapReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.MarkMapReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.MarkMapReqOuterClass.MarkMapReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.MarkMapReqOuterClass.MarkMapReq.parseFrom(payload)
                            ), q);
