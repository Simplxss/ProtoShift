package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.OpenStateChangeNotify, type = 1)
    public class HandlerOpenStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.OpenStateChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.OpenStateChangeNotifyOuterClass.OpenStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.OpenStateChangeNotifyOuterClass.OpenStateChangeNotify.parseFrom(payload)
                            ), q);
