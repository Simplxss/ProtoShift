package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarTeamUpdateNotify, type = 1)
    public class HandlerAvatarTeamUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarTeamUpdateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarTeamUpdateNotifyOuterClass.AvatarTeamUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarTeamUpdateNotifyOuterClass.AvatarTeamUpdateNotify.parseFrom(payload)
                            ), q);
