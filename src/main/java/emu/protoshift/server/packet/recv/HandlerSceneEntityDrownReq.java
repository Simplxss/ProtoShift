package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneEntityDrownReq, type = 1)
    public class HandlerSceneEntityDrownReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneEntityDrownReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneEntityDrownReqOuterClass.SceneEntityDrownReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneEntityDrownReqOuterClass.SceneEntityDrownReq.parseFrom(payload)
                            ), q);
