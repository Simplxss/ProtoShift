package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarDataNotify, type = 1)
    public class HandlerAvatarDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarDataNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarDataNotifyOuterClass.AvatarDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarDataNotifyOuterClass.AvatarDataNotify.parseFrom(payload)
                            ), q);
