package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PrivateChatReq, type = 1)
    public class HandlerPrivateChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PrivateChatReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PrivateChatReqOuterClass.PrivateChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PrivateChatReqOuterClass.PrivateChatReq.parseFrom(payload)
                            ), q);
