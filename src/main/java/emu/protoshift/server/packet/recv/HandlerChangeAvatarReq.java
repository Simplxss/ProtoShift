package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ChangeAvatarReq, type = 1)
    public class HandlerChangeAvatarReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ChangeAvatarReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ChangeAvatarReqOuterClass.ChangeAvatarReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ChangeAvatarReqOuterClass.ChangeAvatarReq.parseFrom(payload)
                            ), q);
