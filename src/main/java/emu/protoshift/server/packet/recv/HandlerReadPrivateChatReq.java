package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ReadPrivateChatReq, type = 1)
    public class HandlerReadPrivateChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ReadPrivateChatReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ReadPrivateChatReqOuterClass.ReadPrivateChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ReadPrivateChatReqOuterClass.ReadPrivateChatReq.parseFrom(payload)
                            ), q);
