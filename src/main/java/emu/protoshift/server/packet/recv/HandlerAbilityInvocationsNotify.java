package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AbilityInvocationsNotify, type = 1)
    public class HandlerAbilityInvocationsNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AbilityInvocationsNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AbilityInvocationsNotifyOuterClass.AbilityInvocationsNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AbilityInvocationsNotifyOuterClass.AbilityInvocationsNotify.parseFrom(payload)
                            ), q);
