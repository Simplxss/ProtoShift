package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ServerGlobalValueChangeNotify, type = 1)
    public class HandlerServerGlobalValueChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ServerGlobalValueChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ServerGlobalValueChangeNotifyOuterClass.ServerGlobalValueChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ServerGlobalValueChangeNotifyOuterClass.ServerGlobalValueChangeNotify.parseFrom(payload)
                            ), q);
