package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AllWidgetBackgroundActiveStateNotify, type = 1)
    public class HandlerAllWidgetBackgroundActiveStateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AllWidgetBackgroundActiveStateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AllWidgetBackgroundActiveStateNotifyOuterClass.AllWidgetBackgroundActiveStateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AllWidgetBackgroundActiveStateNotifyOuterClass.AllWidgetBackgroundActiveStateNotify.parseFrom(payload)
                            ), q);
