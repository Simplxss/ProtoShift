package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerChatCDNotify, type = 1)
    public class HandlerPlayerChatCDNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerChatCDNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerChatCDNotifyOuterClass.PlayerChatCDNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerChatCDNotifyOuterClass.PlayerChatCDNotify.parseFrom(payload)
                            ), q);
