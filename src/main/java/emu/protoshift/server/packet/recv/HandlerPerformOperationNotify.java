package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PerformOperationNotify, type = 1)
    public class HandlerPerformOperationNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PerformOperationNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PerformOperationNotifyOuterClass.PerformOperationNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PerformOperationNotifyOuterClass.PerformOperationNotify.parseFrom(payload)
                            ), q);
