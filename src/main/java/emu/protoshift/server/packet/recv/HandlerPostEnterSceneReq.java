package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PostEnterSceneReq, type = 1)
    public class HandlerPostEnterSceneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PostEnterSceneReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PostEnterSceneReqOuterClass.PostEnterSceneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PostEnterSceneReqOuterClass.PostEnterSceneReq.parseFrom(payload)
                            ), q);
