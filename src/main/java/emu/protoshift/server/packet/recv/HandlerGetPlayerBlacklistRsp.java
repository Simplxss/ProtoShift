package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerBlacklistRsp, type = 1)
    public class HandlerGetPlayerBlacklistRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerBlacklistRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerBlacklistRspOuterClass.GetPlayerBlacklistRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerBlacklistRspOuterClass.GetPlayerBlacklistRsp.parseFrom(payload)
                            ), q);
