package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarFightPropNotify, type = 1)
    public class HandlerAvatarFightPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarFightPropNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarFightPropNotifyOuterClass.AvatarFightPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarFightPropNotifyOuterClass.AvatarFightPropNotify.parseFrom(payload)
                            ), q);
