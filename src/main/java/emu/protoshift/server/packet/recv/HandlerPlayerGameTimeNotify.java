package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerGameTimeNotify, type = 1)
    public class HandlerPlayerGameTimeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerGameTimeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerGameTimeNotifyOuterClass.PlayerGameTimeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerGameTimeNotifyOuterClass.PlayerGameTimeNotify.parseFrom(payload)
                            ), q);
