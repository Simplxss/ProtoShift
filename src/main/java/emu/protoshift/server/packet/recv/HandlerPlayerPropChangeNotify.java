package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerPropChangeNotify, type = 1)
    public class HandlerPlayerPropChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerPropChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerPropChangeNotifyOuterClass.PlayerPropChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerPropChangeNotifyOuterClass.PlayerPropChangeNotify.parseFrom(payload)
                            ), q);
