    ...
    public class HandlerEnterSceneDoneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterSceneDoneReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterSceneDoneReqOuterClass.EnterSceneDoneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterSceneDoneReqOuterClass.EnterSceneDoneReq.parseFrom(payload)
                            ), q);
