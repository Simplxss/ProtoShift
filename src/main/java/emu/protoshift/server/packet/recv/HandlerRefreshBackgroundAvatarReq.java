package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.RefreshBackgroundAvatarReq, type = 1)
    public class HandlerRefreshBackgroundAvatarReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.RefreshBackgroundAvatarReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.RefreshBackgroundAvatarReqOuterClass.RefreshBackgroundAvatarReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.RefreshBackgroundAvatarReqOuterClass.RefreshBackgroundAvatarReq.parseFrom(payload)
                            ), q);
