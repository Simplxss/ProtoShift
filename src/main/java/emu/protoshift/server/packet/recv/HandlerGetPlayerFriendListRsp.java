package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerFriendListRsp, type = 1)
    public class HandlerGetPlayerFriendListRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerFriendListRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerFriendListRspOuterClass.GetPlayerFriendListRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerFriendListRspOuterClass.GetPlayerFriendListRsp.parseFrom(payload)
                            ), q);
