package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetWidgetSlotReq, type = 1)
    public class HandlerSetWidgetSlotReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetWidgetSlotReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetWidgetSlotReqOuterClass.SetWidgetSlotReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetWidgetSlotReqOuterClass.SetWidgetSlotReq.parseFrom(payload)
                            ), q);
