package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.FishBiteReq, type = 1)
    public class HandlerFishBiteReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.FishBiteReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.FishBiteReqOuterClass.FishBiteReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.FishBiteReqOuterClass.FishBiteReq.parseFrom(payload)
                            ), q);
