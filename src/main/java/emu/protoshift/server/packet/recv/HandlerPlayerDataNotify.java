package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerDataNotify, type = 1)
    public class HandlerPlayerDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerDataNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerDataNotifyOuterClass.PlayerDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerDataNotifyOuterClass.PlayerDataNotify.parseFrom(payload)
                            ), q);
