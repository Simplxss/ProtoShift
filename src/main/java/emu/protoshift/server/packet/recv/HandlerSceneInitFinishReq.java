package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneInitFinishReq, type = 1)
    public class HandlerSceneInitFinishReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneInitFinishReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneInitFinishReqOuterClass.SceneInitFinishReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneInitFinishReqOuterClass.SceneInitFinishReq.parseFrom(payload)
                            ), q);
