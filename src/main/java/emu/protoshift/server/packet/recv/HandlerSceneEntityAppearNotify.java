package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneEntityAppearNotify, type = 1)
    public class HandlerSceneEntityAppearNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneEntityAppearNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneEntityAppearNotifyOuterClass.SceneEntityAppearNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneEntityAppearNotifyOuterClass.SceneEntityAppearNotify.parseFrom(payload)
                            ), q);
