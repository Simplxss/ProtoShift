package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneTimeNotify, type = 1)
    public class HandlerSceneTimeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneTimeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneTimeNotifyOuterClass.SceneTimeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneTimeNotifyOuterClass.SceneTimeNotify.parseFrom(payload)
                            ), q);
