package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PrivateChatRsp, type = 1)
    public class HandlerPrivateChatRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PrivateChatRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PrivateChatRspOuterClass.PrivateChatRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PrivateChatRspOuterClass.PrivateChatRsp.parseFrom(payload)
                            ), q);
