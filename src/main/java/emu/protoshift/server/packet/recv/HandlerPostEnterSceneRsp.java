package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PostEnterSceneRsp, type = 1)
    public class HandlerPostEnterSceneRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PostEnterSceneRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PostEnterSceneRspOuterClass.PostEnterSceneRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PostEnterSceneRspOuterClass.PostEnterSceneRsp.parseFrom(payload)
                            ), q);
