package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerAskFriendListReq, type = 1)
    public class HandlerGetPlayerAskFriendListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerAskFriendListReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerAskFriendListReqOuterClass.GetPlayerAskFriendListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerAskFriendListReqOuterClass.GetPlayerAskFriendListReq.parseFrom(payload)
                            ), q);
