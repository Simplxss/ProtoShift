package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetPlayerNameReq, type = 1)
    public class HandlerSetPlayerNameReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetPlayerNameReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetPlayerNameReqOuterClass.SetPlayerNameReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetPlayerNameReqOuterClass.SetPlayerNameReq.parseFrom(payload)
                            ), q);
