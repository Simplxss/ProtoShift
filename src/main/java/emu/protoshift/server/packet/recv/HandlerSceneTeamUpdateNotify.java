package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneTeamUpdateNotify, type = 1)
    public class HandlerSceneTeamUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneTeamUpdateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneTeamUpdateNotifyOuterClass.SceneTeamUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneTeamUpdateNotifyOuterClass.SceneTeamUpdateNotify.parseFrom(payload)
                            ), q);
