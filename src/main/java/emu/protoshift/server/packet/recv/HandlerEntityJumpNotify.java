package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EntityJumpNotify, type = 1)
    public class HandlerEntityJumpNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EntityJumpNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EntityJumpNotifyOuterClass.EntityJumpNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EntityJumpNotifyOuterClass.EntityJumpNotify.parseFrom(payload)
                            ), q);
