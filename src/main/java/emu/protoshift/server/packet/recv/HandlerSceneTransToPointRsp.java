package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneTransToPointRsp, type = 1)
    public class HandlerSceneTransToPointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneTransToPointRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneTransToPointRspOuterClass.SceneTransToPointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneTransToPointRspOuterClass.SceneTransToPointRsp.parseFrom(payload)
                            ), q);
