package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetOpenStateRsp, type = 1)
    public class HandlerSetOpenStateRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetOpenStateRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetOpenStateRspOuterClass.SetOpenStateRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetOpenStateRspOuterClass.SetOpenStateRsp.parseFrom(payload)
                            ), q);
