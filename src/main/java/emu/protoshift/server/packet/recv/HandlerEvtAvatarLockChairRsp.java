package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtAvatarLockChairRsp, type = 1)
    public class HandlerEvtAvatarLockChairRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtAvatarLockChairRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtAvatarLockChairRspOuterClass.EvtAvatarLockChairRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtAvatarLockChairRspOuterClass.EvtAvatarLockChairRsp.parseFrom(payload)
                            ), q);
