package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.HostPlayerNotify, type = 1)
    public class HandlerHostPlayerNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.HostPlayerNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.HostPlayerNotifyOuterClass.HostPlayerNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.HostPlayerNotifyOuterClass.HostPlayerNotify.parseFrom(payload)
                            ), q);
