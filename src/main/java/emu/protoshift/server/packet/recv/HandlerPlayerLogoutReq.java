package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerLogoutReq, type = 1)
    public class HandlerPlayerLogoutReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerLogoutReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerLogoutReqOuterClass.PlayerLogoutReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerLogoutReqOuterClass.PlayerLogoutReq.parseFrom(payload)
                            ), q);
