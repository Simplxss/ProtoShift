package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.OpenStateUpdateNotify, type = 1)
    public class HandlerOpenStateUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.OpenStateUpdateNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.OpenStateUpdateNotifyOuterClass.OpenStateUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.OpenStateUpdateNotifyOuterClass.OpenStateUpdateNotify.parseFrom(payload)
                            ), q);
