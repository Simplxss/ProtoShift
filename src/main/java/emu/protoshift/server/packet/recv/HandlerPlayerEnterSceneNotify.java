    ...
    public class HandlerPlayerEnterSceneNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerEnterSceneNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerEnterSceneNotifyOuterClass.PlayerEnterSceneNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerEnterSceneNotifyOuterClass.PlayerEnterSceneNotify.parseFrom(payload)
                            ), q);
