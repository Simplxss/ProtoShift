package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetPlayerBornDataReq, type = 1)
    public class HandlerSetPlayerBornDataReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetPlayerBornDataReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetPlayerBornDataReqOuterClass.SetPlayerBornDataReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetPlayerBornDataReqOuterClass.SetPlayerBornDataReq.parseFrom(payload)
                            ), q);
