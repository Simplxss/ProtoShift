package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EntityAiKillSelfNotify, type = 1)
    public class HandlerEntityAiKillSelfNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EntityAiKillSelfNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EntityAiKillSelfNotifyOuterClass.EntityAiKillSelfNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EntityAiKillSelfNotifyOuterClass.EntityAiKillSelfNotify.parseFrom(payload)
                            ), q);
