package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.NpcTalkRsp, type = 1)
    public class HandlerNpcTalkRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.NpcTalkRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.NpcTalkRspOuterClass.NpcTalkRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.NpcTalkRspOuterClass.NpcTalkRsp.parseFrom(payload)
                            ), q);
