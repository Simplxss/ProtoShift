package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetOnlinePlayerListReq, type = 1)
    public class HandlerGetOnlinePlayerListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetOnlinePlayerListReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetOnlinePlayerListReqOuterClass.GetOnlinePlayerListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetOnlinePlayerListReqOuterClass.GetOnlinePlayerListReq.parseFrom(payload)
                            ), q);
