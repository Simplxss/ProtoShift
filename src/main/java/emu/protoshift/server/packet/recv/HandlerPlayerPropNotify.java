package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerPropNotify, type = 1)
    public class HandlerPlayerPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerPropNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerPropNotifyOuterClass.PlayerPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerPropNotifyOuterClass.PlayerPropNotify.parseFrom(payload)
                            ), q);
