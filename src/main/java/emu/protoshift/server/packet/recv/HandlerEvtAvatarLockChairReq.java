package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtAvatarLockChairReq, type = 1)
    public class HandlerEvtAvatarLockChairReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtAvatarLockChairReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtAvatarLockChairReqOuterClass.EvtAvatarLockChairReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtAvatarLockChairReqOuterClass.EvtAvatarLockChairReq.parseFrom(payload)
                            ), q);
