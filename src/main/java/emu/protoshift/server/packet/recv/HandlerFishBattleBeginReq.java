package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.FishBattleBeginReq, type = 1)
    public class HandlerFishBattleBeginReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.FishBattleBeginReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.FishBattleBeginReqOuterClass.FishBattleBeginReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.FishBattleBeginReqOuterClass.FishBattleBeginReq.parseFrom(payload)
                            ), q);
