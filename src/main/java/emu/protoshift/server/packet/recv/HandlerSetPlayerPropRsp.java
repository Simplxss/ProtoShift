package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetPlayerPropRsp, type = 1)
    public class HandlerSetPlayerPropRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetPlayerPropRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetPlayerPropRspOuterClass.SetPlayerPropRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetPlayerPropRspOuterClass.SetPlayerPropRsp.parseFrom(payload)
                            ), q);
