package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetSceneAreaReq, type = 1)
    public class HandlerGetSceneAreaReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetSceneAreaReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetSceneAreaReqOuterClass.GetSceneAreaReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetSceneAreaReqOuterClass.GetSceneAreaReq.parseFrom(payload)
                            ), q);
