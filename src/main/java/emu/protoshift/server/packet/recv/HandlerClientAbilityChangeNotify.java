package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ClientAbilityChangeNotify, type = 1)
    public class HandlerClientAbilityChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ClientAbilityChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ClientAbilityChangeNotifyOuterClass.ClientAbilityChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ClientAbilityChangeNotifyOuterClass.ClientAbilityChangeNotify.parseFrom(payload)
                            ), q);
