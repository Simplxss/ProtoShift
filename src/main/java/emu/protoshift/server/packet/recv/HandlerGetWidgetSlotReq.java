    ...
    public class HandlerGetWidgetSlotReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetWidgetSlotReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetWidgetSlotReqOuterClass.GetWidgetSlotReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetWidgetSlotReqOuterClass.GetWidgetSlotReq.parseFrom(payload)
                            ), q);
