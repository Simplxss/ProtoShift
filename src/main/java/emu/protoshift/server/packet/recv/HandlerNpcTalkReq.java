package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.NpcTalkReq, type = 1)
    public class HandlerNpcTalkReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.NpcTalkReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.NpcTalkReqOuterClass.NpcTalkReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.NpcTalkReqOuterClass.NpcTalkReq.parseFrom(payload)
                            ), q);
