package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ChangeServerGlobalValueNotify, type = 1)
    public class HandlerChangeServerGlobalValueNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ChangeServerGlobalValueNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ChangeServerGlobalValueNotifyOuterClass.ChangeServerGlobalValueNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ChangeServerGlobalValueNotifyOuterClass.ChangeServerGlobalValueNotify.parseFrom(payload)
                            ), q);
