package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.WorldPlayerInfoNotify, type = 1)
    public class HandlerWorldPlayerInfoNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WorldPlayerInfoNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WorldPlayerInfoNotifyOuterClass.WorldPlayerInfoNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WorldPlayerInfoNotifyOuterClass.WorldPlayerInfoNotify.parseFrom(payload)
                            ), q);
