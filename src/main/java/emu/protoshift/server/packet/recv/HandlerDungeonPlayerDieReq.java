package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.DungeonPlayerDieReq, type = 1)
    public class HandlerDungeonPlayerDieReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.DungeonPlayerDieReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.DungeonPlayerDieReqOuterClass.DungeonPlayerDieReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.DungeonPlayerDieReqOuterClass.DungeonPlayerDieReq.parseFrom(payload)
                            ), q);
