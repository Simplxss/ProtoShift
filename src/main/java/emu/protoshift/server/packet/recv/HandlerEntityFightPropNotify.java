package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EntityFightPropNotify, type = 1)
    public class HandlerEntityFightPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EntityFightPropNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EntityFightPropNotifyOuterClass.EntityFightPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EntityFightPropNotifyOuterClass.EntityFightPropNotify.parseFrom(payload)
                            ), q);
