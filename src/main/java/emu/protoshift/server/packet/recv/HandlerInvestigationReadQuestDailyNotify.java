package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.InvestigationReadQuestDailyNotify, type = 1)
    public class HandlerInvestigationReadQuestDailyNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.InvestigationReadQuestDailyNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.InvestigationReadQuestDailyNotifyOuterClass.InvestigationReadQuestDailyNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.InvestigationReadQuestDailyNotifyOuterClass.InvestigationReadQuestDailyNotify.parseFrom(payload)
                            ), q);
