package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetSceneAreaRsp, type = 1)
    public class HandlerGetSceneAreaRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetSceneAreaRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetSceneAreaRspOuterClass.GetSceneAreaRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetSceneAreaRspOuterClass.GetSceneAreaRsp.parseFrom(payload)
                            ), q);
