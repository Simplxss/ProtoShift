package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerFriendListReq, type = 1)
    public class HandlerGetPlayerFriendListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerFriendListReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerFriendListReqOuterClass.GetPlayerFriendListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerFriendListReqOuterClass.GetPlayerFriendListReq.parseFrom(payload)
                            ), q);
