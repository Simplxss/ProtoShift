package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.DoSetPlayerBornDataNotify, type = 1)
    public class HandlerDoSetPlayerBornDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.DoSetPlayerBornDataNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.DoSetPlayerBornDataNotifyOuterClass.DoSetPlayerBornDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.DoSetPlayerBornDataNotifyOuterClass.DoSetPlayerBornDataNotify.parseFrom(payload)
                            ), q);
