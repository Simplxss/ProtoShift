package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarAddNotify, type = 1)
    public class HandlerAvatarAddNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarAddNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarAddNotifyOuterClass.AvatarAddNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarAddNotifyOuterClass.AvatarAddNotify.parseFrom(payload)
                            ), q);
