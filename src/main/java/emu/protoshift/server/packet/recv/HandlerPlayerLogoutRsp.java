package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerLogoutRsp, type = 1)
    public class HandlerPlayerLogoutRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerLogoutRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerLogoutRspOuterClass.PlayerLogoutRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerLogoutRspOuterClass.PlayerLogoutRsp.parseFrom(payload)
                            ), q);
