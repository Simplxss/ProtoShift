package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ScenePointUnlockNotify, type = 1)
    public class HandlerScenePointUnlockNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ScenePointUnlockNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ScenePointUnlockNotifyOuterClass.ScenePointUnlockNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ScenePointUnlockNotifyOuterClass.ScenePointUnlockNotify.parseFrom(payload)
                            ), q);
