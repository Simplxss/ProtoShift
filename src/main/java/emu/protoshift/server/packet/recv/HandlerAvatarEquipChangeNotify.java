package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarEquipChangeNotify, type = 1)
    public class HandlerAvatarEquipChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarEquipChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarEquipChangeNotifyOuterClass.AvatarEquipChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarEquipChangeNotifyOuterClass.AvatarEquipChangeNotify.parseFrom(payload)
                            ), q);
