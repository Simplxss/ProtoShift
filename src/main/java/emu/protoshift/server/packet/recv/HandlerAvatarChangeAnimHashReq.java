package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarChangeAnimHashReq, type = 1)
    public class HandlerAvatarChangeAnimHashReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarChangeAnimHashReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarChangeAnimHashReqOuterClass.AvatarChangeAnimHashReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarChangeAnimHashReqOuterClass.AvatarChangeAnimHashReq.parseFrom(payload)
                            ), q);
