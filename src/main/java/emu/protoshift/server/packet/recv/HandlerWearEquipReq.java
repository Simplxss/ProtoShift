package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.WearEquipReq, type = 1)
    public class HandlerWearEquipReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WearEquipReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WearEquipReqOuterClass.WearEquipReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WearEquipReqOuterClass.WearEquipReq.parseFrom(payload)
                            ), q);
