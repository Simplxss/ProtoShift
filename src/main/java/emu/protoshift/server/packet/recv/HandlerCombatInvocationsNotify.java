package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.CombatInvocationsNotify, type = 1)
    public class HandlerCombatInvocationsNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.CombatInvocationsNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.CombatInvocationsNotifyOuterClass.CombatInvocationsNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.CombatInvocationsNotifyOuterClass.CombatInvocationsNotify.parseFrom(payload)
                            ), q);
