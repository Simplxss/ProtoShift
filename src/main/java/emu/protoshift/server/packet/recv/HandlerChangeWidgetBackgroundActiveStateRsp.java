package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ChangeWidgetBackgroundActiveStateRsp, type = 1)
    public class HandlerChangeWidgetBackgroundActiveStateRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ChangeWidgetBackgroundActiveStateRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ChangeWidgetBackgroundActiveStateRspOuterClass.ChangeWidgetBackgroundActiveStateRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ChangeWidgetBackgroundActiveStateRspOuterClass.ChangeWidgetBackgroundActiveStateRsp.parseFrom(payload)
                            ), q);
