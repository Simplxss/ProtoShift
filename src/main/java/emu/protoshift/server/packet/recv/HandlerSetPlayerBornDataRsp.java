package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetPlayerBornDataRsp, type = 1)
    public class HandlerSetPlayerBornDataRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetPlayerBornDataRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetPlayerBornDataRspOuterClass.SetPlayerBornDataRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetPlayerBornDataRspOuterClass.SetPlayerBornDataRsp.parseFrom(payload)
                            ), q);
