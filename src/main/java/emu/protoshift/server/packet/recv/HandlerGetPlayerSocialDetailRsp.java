package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerSocialDetailRsp, type = 1)
    public class HandlerGetPlayerSocialDetailRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerSocialDetailRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerSocialDetailRspOuterClass.GetPlayerSocialDetailRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerSocialDetailRspOuterClass.GetPlayerSocialDetailRsp.parseFrom(payload)
                            ), q);
