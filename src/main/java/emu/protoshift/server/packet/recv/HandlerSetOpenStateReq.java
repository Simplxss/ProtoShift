package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetOpenStateReq, type = 1)
    public class HandlerSetOpenStateReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetOpenStateReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetOpenStateReqOuterClass.SetOpenStateReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetOpenStateReqOuterClass.SetOpenStateReq.parseFrom(payload)
                            ), q);
