package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.ReadPrivateChatRsp, type = 1)
    public class HandlerReadPrivateChatRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.ReadPrivateChatRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.ReadPrivateChatRspOuterClass.ReadPrivateChatRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.ReadPrivateChatRspOuterClass.ReadPrivateChatRsp.parseFrom(payload)
                            ), q);
