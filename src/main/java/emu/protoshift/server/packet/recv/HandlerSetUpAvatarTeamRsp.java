package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetUpAvatarTeamRsp, type = 1)
    public class HandlerSetUpAvatarTeamRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetUpAvatarTeamRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetUpAvatarTeamRspOuterClass.SetUpAvatarTeamRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetUpAvatarTeamRspOuterClass.SetUpAvatarTeamRsp.parseFrom(payload)
                            ), q);
