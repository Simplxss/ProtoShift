package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarChangeAnimHashRsp, type = 1)
    public class HandlerAvatarChangeAnimHashRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarChangeAnimHashRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarChangeAnimHashRspOuterClass.AvatarChangeAnimHashRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarChangeAnimHashRspOuterClass.AvatarChangeAnimHashRsp.parseFrom(payload)
                            ), q);
