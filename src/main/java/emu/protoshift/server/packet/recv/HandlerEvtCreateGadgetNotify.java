package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EvtCreateGadgetNotify, type = 1)
    public class HandlerEvtCreateGadgetNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EvtCreateGadgetNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EvtCreateGadgetNotifyOuterClass.EvtCreateGadgetNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EvtCreateGadgetNotifyOuterClass.EvtCreateGadgetNotify.parseFrom(payload)
                            ), q);
