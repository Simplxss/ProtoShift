package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.EnterSceneDoneRsp, type = 1)
    public class HandlerEnterSceneDoneRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.EnterSceneDoneRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.EnterSceneDoneRspOuterClass.EnterSceneDoneRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.EnterSceneDoneRspOuterClass.EnterSceneDoneRsp.parseFrom(payload)
                            ), q);
