package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PlayerChatNotify, type = 1)
    public class HandlerPlayerChatNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerChatNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerChatNotifyOuterClass.PlayerChatNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerChatNotifyOuterClass.PlayerChatNotify.parseFrom(payload)
                            ), q);
