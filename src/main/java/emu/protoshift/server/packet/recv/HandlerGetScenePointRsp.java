package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetScenePointRsp, type = 1)
    public class HandlerGetScenePointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetScenePointRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetScenePointRspOuterClass.GetScenePointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetScenePointRspOuterClass.GetScenePointRsp.parseFrom(payload)
                            ), q);
