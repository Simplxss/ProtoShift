package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.AvatarFlycloakChangeNotify, type = 1)
    public class HandlerAvatarFlycloakChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.AvatarFlycloakChangeNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.AvatarFlycloakChangeNotifyOuterClass.AvatarFlycloakChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.AvatarFlycloakChangeNotifyOuterClass.AvatarFlycloakChangeNotify.parseFrom(payload)
                            ), q);
