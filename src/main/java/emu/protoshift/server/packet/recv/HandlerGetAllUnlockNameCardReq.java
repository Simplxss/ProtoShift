package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetAllUnlockNameCardReq, type = 1)
    public class HandlerGetAllUnlockNameCardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetAllUnlockNameCardReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetAllUnlockNameCardReqOuterClass.GetAllUnlockNameCardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetAllUnlockNameCardReqOuterClass.GetAllUnlockNameCardReq.parseFrom(payload)
                            ), q);
