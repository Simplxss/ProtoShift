    ...
    public class HandlerPlayerLogoutNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PlayerLogoutNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PlayerLogoutNotifyOuterClass.PlayerLogoutNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PlayerLogoutNotifyOuterClass.PlayerLogoutNotify.parseFrom(payload)
                            ), q);
