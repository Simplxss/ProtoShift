package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SetNameCardReq, type = 1)
    public class HandlerSetNameCardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SetNameCardReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SetNameCardReqOuterClass.SetNameCardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SetNameCardReqOuterClass.SetNameCardReq.parseFrom(payload)
                            ), q);
