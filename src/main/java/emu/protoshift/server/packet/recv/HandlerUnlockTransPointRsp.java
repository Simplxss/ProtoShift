package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.UnlockTransPointRsp, type = 1)
    public class HandlerUnlockTransPointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.UnlockTransPointRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.UnlockTransPointRspOuterClass.UnlockTransPointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.UnlockTransPointRspOuterClass.UnlockTransPointRsp.parseFrom(payload)
                            ), q);
