package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetRecentMpPlayerListReq, type = 1)
    public class HandlerGetRecentMpPlayerListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetRecentMpPlayerListReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetRecentMpPlayerListReqOuterClass.GetRecentMpPlayerListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetRecentMpPlayerListReqOuterClass.GetRecentMpPlayerListReq.parseFrom(payload)
                            ), q);
