package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.GetPlayerSocialDetailReq, type = 1)
    public class HandlerGetPlayerSocialDetailReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.GetPlayerSocialDetailReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.GetPlayerSocialDetailReqOuterClass.GetPlayerSocialDetailReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.GetPlayerSocialDetailReqOuterClass.GetPlayerSocialDetailReq.parseFrom(payload)
                            ), q);
