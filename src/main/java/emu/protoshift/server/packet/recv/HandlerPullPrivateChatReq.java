package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.PullPrivateChatReq, type = 1)
    public class HandlerPullPrivateChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.PullPrivateChatReq, 2), encryptType);
                var q = emu.protoshift.net.oldproto.PullPrivateChatReqOuterClass.PullPrivateChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.PullPrivateChatReqOuterClass.PullPrivateChatReq.parseFrom(payload)
                            ), q);
