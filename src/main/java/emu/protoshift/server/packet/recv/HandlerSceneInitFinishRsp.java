package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.SceneInitFinishRsp, type = 1)
    public class HandlerSceneInitFinishRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.SceneInitFinishRsp, 2), encryptType);
                var q = emu.protoshift.net.oldproto.SceneInitFinishRspOuterClass.SceneInitFinishRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.SceneInitFinishRspOuterClass.SceneInitFinishRsp.parseFrom(payload)
                            ), q);
