package emu.protoshift.server.packet.recv;
    ...
    @Opcodes(value = PacketOpcodes.newOpcodes.WorldDataNotify, type = 1)
    public class HandlerWorldDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.oldOpcodes.WorldDataNotify, 2), encryptType);
                var q = emu.protoshift.net.oldproto.WorldDataNotifyOuterClass.WorldDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                emu.protoshift.net.newproto.WorldDataNotifyOuterClass.WorldDataNotify.parseFrom(payload)
                            ), q);
