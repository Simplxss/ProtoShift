package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WorldPlayerInfoNotify, type = 2)
    public class HandlerWorldPlayerInfoNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WorldPlayerInfoNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.WorldPlayerInfoNotifyOuterClass.WorldPlayerInfoNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WorldPlayerInfoNotifyOuterClass.WorldPlayerInfoNotify.parseFrom(payload)
                            ), p);
