package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ExitTransPointRegionNotify, type = 2)
    public class HandlerExitTransPointRegionNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ExitTransPointRegionNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ExitTransPointRegionNotifyOuterClass.ExitTransPointRegionNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ExitTransPointRegionNotifyOuterClass.ExitTransPointRegionNotify.parseFrom(payload)
                            ), p);
