package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.OpenStateUpdateNotify, type = 2)
    public class HandlerOpenStateUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.OpenStateUpdateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.OpenStateUpdateNotifyOuterClass.OpenStateUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.OpenStateUpdateNotifyOuterClass.OpenStateUpdateNotify.parseFrom(payload)
                            ), p);
