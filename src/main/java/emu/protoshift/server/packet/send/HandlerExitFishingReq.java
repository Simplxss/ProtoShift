package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ExitFishingReq, type = 2)
    public class HandlerExitFishingReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ExitFishingReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.ExitFishingReqOuterClass.ExitFishingReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ExitFishingReqOuterClass.ExitFishingReq.parseFrom(payload)
                            ), p);
