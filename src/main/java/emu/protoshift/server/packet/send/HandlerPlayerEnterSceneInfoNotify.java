package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerEnterSceneInfoNotify, type = 2)
    public class HandlerPlayerEnterSceneInfoNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerEnterSceneInfoNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerEnterSceneInfoNotifyOuterClass.PlayerEnterSceneInfoNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerEnterSceneInfoNotifyOuterClass.PlayerEnterSceneInfoNotify.parseFrom(payload)
                            ), p);
