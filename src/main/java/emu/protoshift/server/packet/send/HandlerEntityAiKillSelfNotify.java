package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EntityAiKillSelfNotify, type = 2)
    public class HandlerEntityAiKillSelfNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EntityAiKillSelfNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EntityAiKillSelfNotifyOuterClass.EntityAiKillSelfNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EntityAiKillSelfNotifyOuterClass.EntityAiKillSelfNotify.parseFrom(payload)
                            ), p);
