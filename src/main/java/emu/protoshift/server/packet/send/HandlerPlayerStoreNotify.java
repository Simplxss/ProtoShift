package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerStoreNotify, type = 2)
    public class HandlerPlayerStoreNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerStoreNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerStoreNotifyOuterClass.PlayerStoreNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerStoreNotifyOuterClass.PlayerStoreNotify.parseFrom(payload)
                            ), p);
