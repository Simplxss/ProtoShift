package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetUpAvatarTeamRsp, type = 2)
    public class HandlerSetUpAvatarTeamRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetUpAvatarTeamRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetUpAvatarTeamRspOuterClass.SetUpAvatarTeamRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetUpAvatarTeamRspOuterClass.SetUpAvatarTeamRsp.parseFrom(payload)
                            ), p);
