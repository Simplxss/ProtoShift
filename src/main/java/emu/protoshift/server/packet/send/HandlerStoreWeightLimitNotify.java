package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.StoreWeightLimitNotify, type = 2)
    public class HandlerStoreWeightLimitNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.StoreWeightLimitNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.StoreWeightLimitNotifyOuterClass.StoreWeightLimitNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.StoreWeightLimitNotifyOuterClass.StoreWeightLimitNotify.parseFrom(payload)
                            ), p);
