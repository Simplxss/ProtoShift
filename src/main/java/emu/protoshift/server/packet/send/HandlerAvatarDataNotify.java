package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarDataNotify, type = 2)
    public class HandlerAvatarDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarDataNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarDataNotifyOuterClass.AvatarDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarDataNotifyOuterClass.AvatarDataNotify.parseFrom(payload)
                            ), p);
