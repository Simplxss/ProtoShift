    ...
    public class HandlerTakeFirstShareRewardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.TakeFirstShareRewardReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.TakeFirstShareRewardReqOuterClass.TakeFirstShareRewardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.TakeFirstShareRewardReqOuterClass.TakeFirstShareRewardReq.parseFrom(payload)
                            ), p);
