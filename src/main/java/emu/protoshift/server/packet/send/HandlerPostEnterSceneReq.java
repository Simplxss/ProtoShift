    ...
    public class HandlerPostEnterSceneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PostEnterSceneReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.PostEnterSceneReqOuterClass.PostEnterSceneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PostEnterSceneReqOuterClass.PostEnterSceneReq.parseFrom(payload)
                            ), p);
