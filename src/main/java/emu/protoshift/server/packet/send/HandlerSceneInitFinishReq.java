package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneInitFinishReq, type = 2)
    public class HandlerSceneInitFinishReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneInitFinishReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneInitFinishReqOuterClass.SceneInitFinishReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneInitFinishReqOuterClass.SceneInitFinishReq.parseFrom(payload)
                            ), p);
