package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneTransToPointRsp, type = 2)
    public class HandlerSceneTransToPointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneTransToPointRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneTransToPointRspOuterClass.SceneTransToPointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneTransToPointRspOuterClass.SceneTransToPointRsp.parseFrom(payload)
                            ), p);
