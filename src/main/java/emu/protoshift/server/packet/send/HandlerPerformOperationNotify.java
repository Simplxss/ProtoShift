package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PerformOperationNotify, type = 2)
    public class HandlerPerformOperationNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PerformOperationNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PerformOperationNotifyOuterClass.PerformOperationNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PerformOperationNotifyOuterClass.PerformOperationNotify.parseFrom(payload)
                            ), p);
