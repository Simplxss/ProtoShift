package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EvtAvatarLockChairReq, type = 2)
    public class HandlerEvtAvatarLockChairReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtAvatarLockChairReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtAvatarLockChairReqOuterClass.EvtAvatarLockChairReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtAvatarLockChairReqOuterClass.EvtAvatarLockChairReq.parseFrom(payload)
                            ), p);
