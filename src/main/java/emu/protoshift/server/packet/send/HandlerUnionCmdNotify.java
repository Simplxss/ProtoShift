package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.UnionCmdNotify, type = 2)
    public class HandlerUnionCmdNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.UnionCmdNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.UnionCmdNotifyOuterClass.UnionCmdNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.UnionCmdNotifyOuterClass.UnionCmdNotify.parseFrom(payload)
                            ), p);
