package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarWearFlycloakRsp, type = 2)
    public class HandlerAvatarWearFlycloakRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarWearFlycloakRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarWearFlycloakRspOuterClass.AvatarWearFlycloakRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarWearFlycloakRspOuterClass.AvatarWearFlycloakRsp.parseFrom(payload)
                            ), p);
