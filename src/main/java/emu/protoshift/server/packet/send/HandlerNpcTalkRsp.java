package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.NpcTalkRsp, type = 2)
    public class HandlerNpcTalkRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.NpcTalkRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.NpcTalkRspOuterClass.NpcTalkRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.NpcTalkRspOuterClass.NpcTalkRsp.parseFrom(payload)
                            ), p);
