package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerTokenReq, type = 2)
    public class HandlerGetPlayerTokenReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerTokenReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerTokenReqOuterClass.GetPlayerTokenReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerTokenReqOuterClass.GetPlayerTokenReq.parseFrom(payload)
                            ), p);
