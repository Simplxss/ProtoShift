package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetPlayerNameReq, type = 2)
    public class HandlerSetPlayerNameReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetPlayerNameReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetPlayerNameReqOuterClass.SetPlayerNameReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetPlayerNameReqOuterClass.SetPlayerNameReq.parseFrom(payload)
                            ), p);
