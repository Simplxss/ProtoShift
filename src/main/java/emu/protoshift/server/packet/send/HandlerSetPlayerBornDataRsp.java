package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetPlayerBornDataRsp, type = 2)
    public class HandlerSetPlayerBornDataRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetPlayerBornDataRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetPlayerBornDataRspOuterClass.SetPlayerBornDataRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetPlayerBornDataRspOuterClass.SetPlayerBornDataRsp.parseFrom(payload)
                            ), p);
