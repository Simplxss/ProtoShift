package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AbilityInvocationsNotify, type = 2)
    public class HandlerAbilityInvocationsNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AbilityInvocationsNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AbilityInvocationsNotifyOuterClass.AbilityInvocationsNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AbilityInvocationsNotifyOuterClass.AbilityInvocationsNotify.parseFrom(payload)
                            ), p);
