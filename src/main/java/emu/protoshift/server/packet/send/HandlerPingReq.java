    ...
    public class HandlerPingReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PingReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.PingReqOuterClass.PingReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PingReqOuterClass.PingReq.parseFrom(payload)
                            ), p);
