package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerSocialDetailReq, type = 2)
    public class HandlerGetPlayerSocialDetailReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerSocialDetailReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerSocialDetailReqOuterClass.GetPlayerSocialDetailReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerSocialDetailReqOuterClass.GetPlayerSocialDetailReq.parseFrom(payload)
                            ), p);
