package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetOpenStateRsp, type = 2)
    public class HandlerSetOpenStateRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetOpenStateRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetOpenStateRspOuterClass.SetOpenStateRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetOpenStateRspOuterClass.SetOpenStateRsp.parseFrom(payload)
                            ), p);
