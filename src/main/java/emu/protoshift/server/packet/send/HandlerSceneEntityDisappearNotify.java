    ...
    public class HandlerSceneEntityDisappearNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneEntityDisappearNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneEntityDisappearNotifyOuterClass.SceneEntityDisappearNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneEntityDisappearNotifyOuterClass.SceneEntityDisappearNotify.parseFrom(payload)
                            ), p);
