package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarFightPropUpdateNotify, type = 2)
    public class HandlerAvatarFightPropUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarFightPropUpdateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarFightPropUpdateNotifyOuterClass.AvatarFightPropUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarFightPropUpdateNotifyOuterClass.AvatarFightPropUpdateNotify.parseFrom(payload)
                            ), p);
