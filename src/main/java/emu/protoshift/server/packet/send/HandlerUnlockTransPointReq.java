package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.UnlockTransPointReq, type = 2)
    public class HandlerUnlockTransPointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.UnlockTransPointReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.UnlockTransPointReqOuterClass.UnlockTransPointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.UnlockTransPointReqOuterClass.UnlockTransPointReq.parseFrom(payload)
                            ), p);
