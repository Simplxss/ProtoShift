    ...
    public class HandlerAvatarLifeStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarLifeStateChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarLifeStateChangeNotifyOuterClass.AvatarLifeStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarLifeStateChangeNotifyOuterClass.AvatarLifeStateChangeNotify.parseFrom(payload)
                            ), p);
