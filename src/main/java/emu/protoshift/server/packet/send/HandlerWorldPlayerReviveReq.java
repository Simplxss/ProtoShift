package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WorldPlayerReviveReq, type = 2)
    public class HandlerWorldPlayerReviveReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WorldPlayerReviveReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.WorldPlayerReviveReqOuterClass.WorldPlayerReviveReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WorldPlayerReviveReqOuterClass.WorldPlayerReviveReq.parseFrom(payload)
                            ), p);
