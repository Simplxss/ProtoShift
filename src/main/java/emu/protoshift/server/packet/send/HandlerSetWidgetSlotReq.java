package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetWidgetSlotReq, type = 2)
    public class HandlerSetWidgetSlotReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetWidgetSlotReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetWidgetSlotReqOuterClass.SetWidgetSlotReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetWidgetSlotReqOuterClass.SetWidgetSlotReq.parseFrom(payload)
                            ), p);
