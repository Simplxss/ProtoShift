package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EnterSceneDoneRsp, type = 2)
    public class HandlerEnterSceneDoneRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterSceneDoneRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterSceneDoneRspOuterClass.EnterSceneDoneRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterSceneDoneRspOuterClass.EnterSceneDoneRsp.parseFrom(payload)
                            ), p);
