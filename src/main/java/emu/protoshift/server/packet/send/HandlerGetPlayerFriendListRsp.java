package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerFriendListRsp, type = 2)
    public class HandlerGetPlayerFriendListRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerFriendListRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerFriendListRspOuterClass.GetPlayerFriendListRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerFriendListRspOuterClass.GetPlayerFriendListRsp.parseFrom(payload)
                            ), p);
