    ...
    public class HandlerReadPrivateChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ReadPrivateChatReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.ReadPrivateChatReqOuterClass.ReadPrivateChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ReadPrivateChatReqOuterClass.ReadPrivateChatReq.parseFrom(payload)
                            ), p);
