package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.OpenStateChangeNotify, type = 2)
    public class HandlerOpenStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.OpenStateChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.OpenStateChangeNotifyOuterClass.OpenStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.OpenStateChangeNotifyOuterClass.OpenStateChangeNotify.parseFrom(payload)
                            ), p);
