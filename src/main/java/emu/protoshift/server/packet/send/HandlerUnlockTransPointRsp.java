    ...
    public class HandlerUnlockTransPointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.UnlockTransPointRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.UnlockTransPointRspOuterClass.UnlockTransPointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.UnlockTransPointRspOuterClass.UnlockTransPointRsp.parseFrom(payload)
                            ), p);
