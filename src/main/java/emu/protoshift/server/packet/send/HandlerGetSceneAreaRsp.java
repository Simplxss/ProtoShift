package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetSceneAreaRsp, type = 2)
    public class HandlerGetSceneAreaRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetSceneAreaRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetSceneAreaRspOuterClass.GetSceneAreaRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetSceneAreaRspOuterClass.GetSceneAreaRsp.parseFrom(payload)
                            ), p);
