package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetScenePointReq, type = 2)
    public class HandlerGetScenePointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetScenePointReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetScenePointReqOuterClass.GetScenePointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetScenePointReqOuterClass.GetScenePointReq.parseFrom(payload)
                            ), p);
