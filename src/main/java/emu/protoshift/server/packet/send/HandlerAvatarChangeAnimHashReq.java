package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarChangeAnimHashReq, type = 2)
    public class HandlerAvatarChangeAnimHashReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarChangeAnimHashReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarChangeAnimHashReqOuterClass.AvatarChangeAnimHashReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarChangeAnimHashReqOuterClass.AvatarChangeAnimHashReq.parseFrom(payload)
                            ), p);
