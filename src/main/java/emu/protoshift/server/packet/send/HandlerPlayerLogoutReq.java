package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerLogoutReq, type = 2)
    public class HandlerPlayerLogoutReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerLogoutReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerLogoutReqOuterClass.PlayerLogoutReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerLogoutReqOuterClass.PlayerLogoutReq.parseFrom(payload)
                            ), p);
