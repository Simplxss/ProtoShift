    ...
    public class HandlerMarkMapReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.MarkMapReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.MarkMapReqOuterClass.MarkMapReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.MarkMapReqOuterClass.MarkMapReq.parseFrom(payload)
                            ), p);
