package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetWidgetSlotRsp, type = 2)
    public class HandlerSetWidgetSlotRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetWidgetSlotRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetWidgetSlotRspOuterClass.SetWidgetSlotRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetWidgetSlotRspOuterClass.SetWidgetSlotRsp.parseFrom(payload)
                            ), p);
