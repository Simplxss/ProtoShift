package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetNameCardReq, type = 2)
    public class HandlerSetNameCardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetNameCardReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetNameCardReqOuterClass.SetNameCardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetNameCardReqOuterClass.SetNameCardReq.parseFrom(payload)
                            ), p);
