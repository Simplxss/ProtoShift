package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneTeamUpdateNotify, type = 2)
    public class HandlerSceneTeamUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneTeamUpdateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneTeamUpdateNotifyOuterClass.SceneTeamUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneTeamUpdateNotifyOuterClass.SceneTeamUpdateNotify.parseFrom(payload)
                            ), p);
