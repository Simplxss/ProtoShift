    ...
    public class HandlerLeaveSceneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.LeaveSceneReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.LeaveSceneReqOuterClass.LeaveSceneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.LeaveSceneReqOuterClass.LeaveSceneReq.parseFrom(payload)
                            ), p);
