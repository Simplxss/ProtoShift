package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PullRecentChatReq, type = 2)
    public class HandlerPullRecentChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PullRecentChatReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.PullRecentChatReqOuterClass.PullRecentChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PullRecentChatReqOuterClass.PullRecentChatReq.parseFrom(payload)
                            ), p);
