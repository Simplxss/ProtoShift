package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EntityFightPropUpdateNotify, type = 2)
    public class HandlerEntityFightPropUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EntityFightPropUpdateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EntityFightPropUpdateNotifyOuterClass.EntityFightPropUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EntityFightPropUpdateNotifyOuterClass.EntityFightPropUpdateNotify.parseFrom(payload)
                            ), p);
