package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EvtCostStaminaNotify, type = 2)
    public class HandlerEvtCostStaminaNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtCostStaminaNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtCostStaminaNotifyOuterClass.EvtCostStaminaNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtCostStaminaNotifyOuterClass.EvtCostStaminaNotify.parseFrom(payload)
                            ), p);
