package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.InvestigationReadQuestDailyNotify, type = 2)
    public class HandlerInvestigationReadQuestDailyNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.InvestigationReadQuestDailyNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.InvestigationReadQuestDailyNotifyOuterClass.InvestigationReadQuestDailyNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.InvestigationReadQuestDailyNotifyOuterClass.InvestigationReadQuestDailyNotify.parseFrom(payload)
                            ), p);
