package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetAllUnlockNameCardRsp, type = 2)
    public class HandlerGetAllUnlockNameCardRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetAllUnlockNameCardRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetAllUnlockNameCardRspOuterClass.GetAllUnlockNameCardRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetAllUnlockNameCardRspOuterClass.GetAllUnlockNameCardRsp.parseFrom(payload)
                            ), p);
