package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EvtDestroyServerGadgetNotify, type = 2)
    public class HandlerEvtDestroyServerGadgetNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtDestroyServerGadgetNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtDestroyServerGadgetNotifyOuterClass.EvtDestroyServerGadgetNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtDestroyServerGadgetNotifyOuterClass.EvtDestroyServerGadgetNotify.parseFrom(payload)
                            ), p);
