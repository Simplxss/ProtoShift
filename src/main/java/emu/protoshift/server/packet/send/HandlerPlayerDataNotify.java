package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerDataNotify, type = 2)
    public class HandlerPlayerDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerDataNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerDataNotifyOuterClass.PlayerDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerDataNotifyOuterClass.PlayerDataNotify.parseFrom(payload)
                            ), p);
