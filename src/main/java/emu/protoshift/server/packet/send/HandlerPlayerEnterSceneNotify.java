package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerEnterSceneNotify, type = 2)
    public class HandlerPlayerEnterSceneNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerEnterSceneNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerEnterSceneNotifyOuterClass.PlayerEnterSceneNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerEnterSceneNotifyOuterClass.PlayerEnterSceneNotify.parseFrom(payload)
                            ), p);
