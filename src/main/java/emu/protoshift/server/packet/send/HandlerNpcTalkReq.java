package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.NpcTalkReq, type = 2)
    public class HandlerNpcTalkReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.NpcTalkReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.NpcTalkReqOuterClass.NpcTalkReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.NpcTalkReqOuterClass.NpcTalkReq.parseFrom(payload)
                            ), p);
