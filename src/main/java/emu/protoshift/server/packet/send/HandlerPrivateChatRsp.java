package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PrivateChatRsp, type = 2)
    public class HandlerPrivateChatRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PrivateChatRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.PrivateChatRspOuterClass.PrivateChatRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PrivateChatRspOuterClass.PrivateChatRsp.parseFrom(payload)
                            ), p);
