package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarTeamUpdateNotify, type = 2)
    public class HandlerAvatarTeamUpdateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarTeamUpdateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarTeamUpdateNotifyOuterClass.AvatarTeamUpdateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarTeamUpdateNotifyOuterClass.AvatarTeamUpdateNotify.parseFrom(payload)
                            ), p);
