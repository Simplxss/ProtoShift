package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ChangeAvatarReq, type = 2)
    public class HandlerChangeAvatarReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ChangeAvatarReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.ChangeAvatarReqOuterClass.ChangeAvatarReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ChangeAvatarReqOuterClass.ChangeAvatarReq.parseFrom(payload)
                            ), p);
