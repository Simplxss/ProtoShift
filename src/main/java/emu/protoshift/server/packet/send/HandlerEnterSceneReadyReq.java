package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EnterSceneReadyReq, type = 2)
    public class HandlerEnterSceneReadyReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterSceneReadyReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterSceneReadyReqOuterClass.EnterSceneReadyReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterSceneReadyReqOuterClass.EnterSceneReadyReq.parseFrom(payload)
                            ), p);
