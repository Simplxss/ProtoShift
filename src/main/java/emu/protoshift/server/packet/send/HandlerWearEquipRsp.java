package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WearEquipRsp, type = 2)
    public class HandlerWearEquipRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WearEquipRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.WearEquipRspOuterClass.WearEquipRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WearEquipRspOuterClass.WearEquipRsp.parseFrom(payload)
                            ), p);
