package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WidgetSlotChangeNotify, type = 2)
    public class HandlerWidgetSlotChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WidgetSlotChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.WidgetSlotChangeNotifyOuterClass.WidgetSlotChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WidgetSlotChangeNotifyOuterClass.WidgetSlotChangeNotify.parseFrom(payload)
                            ), p);
