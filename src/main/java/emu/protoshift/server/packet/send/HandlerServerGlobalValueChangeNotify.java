package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ServerGlobalValueChangeNotify, type = 2)
    public class HandlerServerGlobalValueChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ServerGlobalValueChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ServerGlobalValueChangeNotifyOuterClass.ServerGlobalValueChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ServerGlobalValueChangeNotifyOuterClass.ServerGlobalValueChangeNotify.parseFrom(payload)
                            ), p);
