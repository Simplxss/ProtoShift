    ...
    public class HandlerPlayerLoginRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerLoginRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerLoginRspOuterClass.PlayerLoginRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerLoginRspOuterClass.PlayerLoginRsp.parseFrom(payload)
                            ), p);
