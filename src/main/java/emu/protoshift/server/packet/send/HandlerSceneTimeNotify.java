package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneTimeNotify, type = 2)
    public class HandlerSceneTimeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneTimeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneTimeNotifyOuterClass.SceneTimeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneTimeNotifyOuterClass.SceneTimeNotify.parseFrom(payload)
                            ), p);
