    ...
    public class HandlerPlayerGameTimeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerGameTimeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerGameTimeNotifyOuterClass.PlayerGameTimeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerGameTimeNotifyOuterClass.PlayerGameTimeNotify.parseFrom(payload)
                            ), p);
