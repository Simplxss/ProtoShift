package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneInitFinishRsp, type = 2)
    public class HandlerSceneInitFinishRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneInitFinishRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneInitFinishRspOuterClass.SceneInitFinishRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneInitFinishRspOuterClass.SceneInitFinishRsp.parseFrom(payload)
                            ), p);
