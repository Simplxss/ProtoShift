package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarFightPropNotify, type = 2)
    public class HandlerAvatarFightPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarFightPropNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarFightPropNotifyOuterClass.AvatarFightPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarFightPropNotifyOuterClass.AvatarFightPropNotify.parseFrom(payload)
                            ), p);
