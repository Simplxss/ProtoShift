package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneDataNotify, type = 2)
    public class HandlerSceneDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneDataNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneDataNotifyOuterClass.SceneDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneDataNotifyOuterClass.SceneDataNotify.parseFrom(payload)
                            ), p);
