package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EnterSceneReadyRsp, type = 2)
    public class HandlerEnterSceneReadyRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterSceneReadyRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterSceneReadyRspOuterClass.EnterSceneReadyRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterSceneReadyRspOuterClass.EnterSceneReadyRsp.parseFrom(payload)
                            ), p);
