    ...
    public class HandlerFishBattleBeginReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.FishBattleBeginReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.FishBattleBeginReqOuterClass.FishBattleBeginReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.FishBattleBeginReqOuterClass.FishBattleBeginReq.parseFrom(payload)
                            ), p);
