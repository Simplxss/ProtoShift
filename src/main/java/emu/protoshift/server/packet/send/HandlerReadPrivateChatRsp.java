    ...
    public class HandlerReadPrivateChatRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ReadPrivateChatRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.ReadPrivateChatRspOuterClass.ReadPrivateChatRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ReadPrivateChatRspOuterClass.ReadPrivateChatRsp.parseFrom(payload)
                            ), p);
