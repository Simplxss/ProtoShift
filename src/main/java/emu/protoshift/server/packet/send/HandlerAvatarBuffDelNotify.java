    ...
    public class HandlerAvatarBuffDelNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarBuffDelNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarBuffDelNotifyOuterClass.AvatarBuffDelNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarBuffDelNotifyOuterClass.AvatarBuffDelNotify.parseFrom(payload)
                            ), p);
