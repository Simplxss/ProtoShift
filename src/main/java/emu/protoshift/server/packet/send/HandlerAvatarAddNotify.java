    ...
    public class HandlerAvatarAddNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarAddNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarAddNotifyOuterClass.AvatarAddNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarAddNotifyOuterClass.AvatarAddNotify.parseFrom(payload)
                            ), p);
