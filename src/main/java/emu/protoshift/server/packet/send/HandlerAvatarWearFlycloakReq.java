package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarWearFlycloakReq, type = 2)
    public class HandlerAvatarWearFlycloakReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarWearFlycloakReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarWearFlycloakReqOuterClass.AvatarWearFlycloakReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarWearFlycloakReqOuterClass.AvatarWearFlycloakReq.parseFrom(payload)
                            ), p);
