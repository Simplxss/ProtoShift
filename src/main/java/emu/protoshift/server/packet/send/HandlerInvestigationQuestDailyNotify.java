package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.InvestigationQuestDailyNotify, type = 2)
    public class HandlerInvestigationQuestDailyNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.InvestigationQuestDailyNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.InvestigationQuestDailyNotifyOuterClass.InvestigationQuestDailyNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.InvestigationQuestDailyNotifyOuterClass.InvestigationQuestDailyNotify.parseFrom(payload)
                            ), p);
