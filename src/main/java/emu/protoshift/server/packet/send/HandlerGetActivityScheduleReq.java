    ...
    public class HandlerGetActivityScheduleReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetActivityScheduleReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetActivityScheduleReqOuterClass.GetActivityScheduleReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetActivityScheduleReqOuterClass.GetActivityScheduleReq.parseFrom(payload)
                            ), p);
