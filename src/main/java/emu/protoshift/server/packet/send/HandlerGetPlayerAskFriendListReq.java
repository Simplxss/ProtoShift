package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerAskFriendListReq, type = 2)
    public class HandlerGetPlayerAskFriendListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerAskFriendListReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerAskFriendListReqOuterClass.GetPlayerAskFriendListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerAskFriendListReqOuterClass.GetPlayerAskFriendListReq.parseFrom(payload)
                            ), p);
