package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarEquipChangeNotify, type = 2)
    public class HandlerAvatarEquipChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarEquipChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarEquipChangeNotifyOuterClass.AvatarEquipChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarEquipChangeNotifyOuterClass.AvatarEquipChangeNotify.parseFrom(payload)
                            ), p);
