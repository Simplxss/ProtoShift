    ...
    public class HandlerClientAbilityChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ClientAbilityChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ClientAbilityChangeNotifyOuterClass.ClientAbilityChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ClientAbilityChangeNotifyOuterClass.ClientAbilityChangeNotify.parseFrom(payload)
                            ), p);
