    ...
    public class HandlerPullPrivateChatReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PullPrivateChatReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.PullPrivateChatReqOuterClass.PullPrivateChatReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PullPrivateChatReqOuterClass.PullPrivateChatReq.parseFrom(payload)
                            ), p);
