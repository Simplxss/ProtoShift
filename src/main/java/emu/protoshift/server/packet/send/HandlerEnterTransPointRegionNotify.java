    ...
    public class HandlerEnterTransPointRegionNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterTransPointRegionNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterTransPointRegionNotifyOuterClass.EnterTransPointRegionNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterTransPointRegionNotifyOuterClass.EnterTransPointRegionNotify.parseFrom(payload)
                            ), p);
