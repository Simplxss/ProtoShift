package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetUpAvatarTeamReq, type = 2)
    public class HandlerSetUpAvatarTeamReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetUpAvatarTeamReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetUpAvatarTeamReqOuterClass.SetUpAvatarTeamReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetUpAvatarTeamReqOuterClass.SetUpAvatarTeamReq.parseFrom(payload)
                            ), p);
