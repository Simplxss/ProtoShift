    ...
    public class HandlerAllWidgetBackgroundActiveStateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AllWidgetBackgroundActiveStateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AllWidgetBackgroundActiveStateNotifyOuterClass.AllWidgetBackgroundActiveStateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AllWidgetBackgroundActiveStateNotifyOuterClass.AllWidgetBackgroundActiveStateNotify.parseFrom(payload)
                            ), p);
