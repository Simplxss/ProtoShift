package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WidgetDoBagRsp, type = 2)
    public class HandlerWidgetDoBagRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WidgetDoBagRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.WidgetDoBagRspOuterClass.WidgetDoBagRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WidgetDoBagRspOuterClass.WidgetDoBagRsp.parseFrom(payload)
                            ), p);
