package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarDieAnimationEndReq, type = 2)
    public class HandlerAvatarDieAnimationEndReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarDieAnimationEndReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarDieAnimationEndReqOuterClass.AvatarDieAnimationEndReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarDieAnimationEndReqOuterClass.AvatarDieAnimationEndReq.parseFrom(payload)
                            ), p);
