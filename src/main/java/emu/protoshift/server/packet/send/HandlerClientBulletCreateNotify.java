package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ClientBulletCreateNotify, type = 2)
    public class HandlerClientBulletCreateNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ClientBulletCreateNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ClientBulletCreateNotifyOuterClass.ClientBulletCreateNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ClientBulletCreateNotifyOuterClass.ClientBulletCreateNotify.parseFrom(payload)
                            ), p);
