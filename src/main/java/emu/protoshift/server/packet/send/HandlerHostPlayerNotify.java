package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.HostPlayerNotify, type = 2)
    public class HandlerHostPlayerNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.HostPlayerNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.HostPlayerNotifyOuterClass.HostPlayerNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.HostPlayerNotifyOuterClass.HostPlayerNotify.parseFrom(payload)
                            ), p);
