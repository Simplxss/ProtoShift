package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerPropChangeNotify, type = 2)
    public class HandlerPlayerPropChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerPropChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerPropChangeNotifyOuterClass.PlayerPropChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerPropChangeNotifyOuterClass.PlayerPropChangeNotify.parseFrom(payload)
                            ), p);
