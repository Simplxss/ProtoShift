package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarChangeAnimHashRsp, type = 2)
    public class HandlerAvatarChangeAnimHashRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarChangeAnimHashRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarChangeAnimHashRspOuterClass.AvatarChangeAnimHashRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarChangeAnimHashRspOuterClass.AvatarChangeAnimHashRsp.parseFrom(payload)
                            ), p);
