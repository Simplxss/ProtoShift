package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ChangeServerGlobalValueNotify, type = 2)
    public class HandlerChangeServerGlobalValueNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ChangeServerGlobalValueNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ChangeServerGlobalValueNotifyOuterClass.ChangeServerGlobalValueNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ChangeServerGlobalValueNotifyOuterClass.ChangeServerGlobalValueNotify.parseFrom(payload)
                            ), p);
