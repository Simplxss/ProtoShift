package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.RefreshBackgroundAvatarReq, type = 2)
    public class HandlerRefreshBackgroundAvatarReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.RefreshBackgroundAvatarReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.RefreshBackgroundAvatarReqOuterClass.RefreshBackgroundAvatarReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.RefreshBackgroundAvatarReqOuterClass.RefreshBackgroundAvatarReq.parseFrom(payload)
                            ), p);
