package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerSocialDetailRsp, type = 2)
    public class HandlerGetPlayerSocialDetailRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerSocialDetailRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerSocialDetailRspOuterClass.GetPlayerSocialDetailRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerSocialDetailRspOuterClass.GetPlayerSocialDetailRsp.parseFrom(payload)
                            ), p);
