    ...
    public class HandlerPostEnterSceneRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PostEnterSceneRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.PostEnterSceneRspOuterClass.PostEnterSceneRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PostEnterSceneRspOuterClass.PostEnterSceneRsp.parseFrom(payload)
                            ), p);
