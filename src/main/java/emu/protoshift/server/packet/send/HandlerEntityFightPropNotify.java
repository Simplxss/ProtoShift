package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EntityFightPropNotify, type = 2)
    public class HandlerEntityFightPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EntityFightPropNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EntityFightPropNotifyOuterClass.EntityFightPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EntityFightPropNotifyOuterClass.EntityFightPropNotify.parseFrom(payload)
                            ), p);
