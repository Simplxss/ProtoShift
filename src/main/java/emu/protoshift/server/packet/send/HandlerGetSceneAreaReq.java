package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetSceneAreaReq, type = 2)
    public class HandlerGetSceneAreaReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetSceneAreaReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetSceneAreaReqOuterClass.GetSceneAreaReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetSceneAreaReqOuterClass.GetSceneAreaReq.parseFrom(payload)
                            ), p);
