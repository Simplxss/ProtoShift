package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AddBackupAvatarTeamReq, type = 2)
    public class HandlerAddBackupAvatarTeamReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AddBackupAvatarTeamReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.AddBackupAvatarTeamReqOuterClass.AddBackupAvatarTeamReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AddBackupAvatarTeamReqOuterClass.AddBackupAvatarTeamReq.parseFrom(payload)
                            ), p);
