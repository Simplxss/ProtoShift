package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EvtCreateGadgetNotify, type = 2)
    public class HandlerEvtCreateGadgetNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtCreateGadgetNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtCreateGadgetNotifyOuterClass.EvtCreateGadgetNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtCreateGadgetNotifyOuterClass.EvtCreateGadgetNotify.parseFrom(payload)
                            ), p);
