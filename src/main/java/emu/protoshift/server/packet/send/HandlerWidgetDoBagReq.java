package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.WidgetDoBagReq, type = 2)
    public class HandlerWidgetDoBagReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.WidgetDoBagReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.WidgetDoBagReqOuterClass.WidgetDoBagReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.WidgetDoBagReqOuterClass.WidgetDoBagReq.parseFrom(payload)
                            ), p);
