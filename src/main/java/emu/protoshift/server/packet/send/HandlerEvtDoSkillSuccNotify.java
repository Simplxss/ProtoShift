    ...
    public class HandlerEvtDoSkillSuccNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtDoSkillSuccNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtDoSkillSuccNotifyOuterClass.EvtDoSkillSuccNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtDoSkillSuccNotifyOuterClass.EvtDoSkillSuccNotify.parseFrom(payload)
                            ), p);
