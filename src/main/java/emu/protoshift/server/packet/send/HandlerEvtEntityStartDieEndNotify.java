    ...
    public class HandlerEvtEntityStartDieEndNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtEntityStartDieEndNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtEntityStartDieEndNotifyOuterClass.EvtEntityStartDieEndNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtEntityStartDieEndNotifyOuterClass.EvtEntityStartDieEndNotify.parseFrom(payload)
                            ), p);
