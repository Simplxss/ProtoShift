    ...
    public class HandlerEvtDestroyGadgetNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtDestroyGadgetNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtDestroyGadgetNotifyOuterClass.EvtDestroyGadgetNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtDestroyGadgetNotifyOuterClass.EvtDestroyGadgetNotify.parseFrom(payload)
                            ), p);
