package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarBuffAddNotify, type = 2)
    public class HandlerAvatarBuffAddNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarBuffAddNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarBuffAddNotifyOuterClass.AvatarBuffAddNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarBuffAddNotifyOuterClass.AvatarBuffAddNotify.parseFrom(payload)
                            ), p);
