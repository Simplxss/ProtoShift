package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarExpeditionAllDataReq, type = 2)
    public class HandlerAvatarExpeditionAllDataReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarExpeditionAllDataReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarExpeditionAllDataReqOuterClass.AvatarExpeditionAllDataReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarExpeditionAllDataReqOuterClass.AvatarExpeditionAllDataReq.parseFrom(payload)
                            ), p);
