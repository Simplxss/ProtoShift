package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerFriendListReq, type = 2)
    public class HandlerGetPlayerFriendListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerFriendListReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerFriendListReqOuterClass.GetPlayerFriendListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerFriendListReqOuterClass.GetPlayerFriendListReq.parseFrom(payload)
                            ), p);
