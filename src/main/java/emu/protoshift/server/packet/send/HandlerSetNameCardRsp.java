package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetNameCardRsp, type = 2)
    public class HandlerSetNameCardRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetNameCardRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetNameCardRspOuterClass.SetNameCardRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetNameCardRspOuterClass.SetNameCardRsp.parseFrom(payload)
                            ), p);
