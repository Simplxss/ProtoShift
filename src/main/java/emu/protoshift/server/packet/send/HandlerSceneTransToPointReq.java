package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneTransToPointReq, type = 2)
    public class HandlerSceneTransToPointReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneTransToPointReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneTransToPointReqOuterClass.SceneTransToPointReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneTransToPointReqOuterClass.SceneTransToPointReq.parseFrom(payload)
                            ), p);
