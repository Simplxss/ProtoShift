package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetScenePointRsp, type = 2)
    public class HandlerGetScenePointRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetScenePointRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetScenePointRspOuterClass.GetScenePointRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetScenePointRspOuterClass.GetScenePointRsp.parseFrom(payload)
                            ), p);
