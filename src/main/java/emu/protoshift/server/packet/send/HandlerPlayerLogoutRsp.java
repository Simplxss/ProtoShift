package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerLogoutRsp, type = 2)
    public class HandlerPlayerLogoutRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerLogoutRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerLogoutRspOuterClass.PlayerLogoutRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerLogoutRspOuterClass.PlayerLogoutRsp.parseFrom(payload)
                            ), p);
