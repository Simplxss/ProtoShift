package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EnterSceneDoneReq, type = 2)
    public class HandlerEnterSceneDoneReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterSceneDoneReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterSceneDoneReqOuterClass.EnterSceneDoneReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterSceneDoneReqOuterClass.EnterSceneDoneReq.parseFrom(payload)
                            ), p);
