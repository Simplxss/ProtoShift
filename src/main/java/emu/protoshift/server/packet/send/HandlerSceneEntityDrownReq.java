package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SceneEntityDrownReq, type = 2)
    public class HandlerSceneEntityDrownReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SceneEntityDrownReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SceneEntityDrownReqOuterClass.SceneEntityDrownReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SceneEntityDrownReqOuterClass.SceneEntityDrownReq.parseFrom(payload)
                            ), p);
