package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EvtAvatarLockChairRsp, type = 2)
    public class HandlerEvtAvatarLockChairRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EvtAvatarLockChairRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.EvtAvatarLockChairRspOuterClass.EvtAvatarLockChairRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EvtAvatarLockChairRspOuterClass.EvtAvatarLockChairRsp.parseFrom(payload)
                            ), p);
