package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.DungeonPlayerDieReq, type = 2)
    public class HandlerDungeonPlayerDieReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.DungeonPlayerDieReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.DungeonPlayerDieReqOuterClass.DungeonPlayerDieReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.DungeonPlayerDieReqOuterClass.DungeonPlayerDieReq.parseFrom(payload)
                            ), p);
