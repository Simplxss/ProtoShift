package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerPropNotify, type = 2)
    public class HandlerPlayerPropNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerPropNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerPropNotifyOuterClass.PlayerPropNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerPropNotifyOuterClass.PlayerPropNotify.parseFrom(payload)
                            ), p);
