package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetOpenStateReq, type = 2)
    public class HandlerSetOpenStateReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetOpenStateReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetOpenStateReqOuterClass.SetOpenStateReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetOpenStateReqOuterClass.SetOpenStateReq.parseFrom(payload)
                            ), p);
