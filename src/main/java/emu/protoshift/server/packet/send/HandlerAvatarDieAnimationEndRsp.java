package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarDieAnimationEndRsp, type = 2)
    public class HandlerAvatarDieAnimationEndRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarDieAnimationEndRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarDieAnimationEndRspOuterClass.AvatarDieAnimationEndRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarDieAnimationEndRspOuterClass.AvatarDieAnimationEndRsp.parseFrom(payload)
                            ), p);
