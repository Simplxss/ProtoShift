    ...
    public class HandlerDungeonPlayerDieRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.DungeonPlayerDieRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.DungeonPlayerDieRspOuterClass.DungeonPlayerDieRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.DungeonPlayerDieRspOuterClass.DungeonPlayerDieRsp.parseFrom(payload)
                            ), p);
