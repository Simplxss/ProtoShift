package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.DoSetPlayerBornDataNotify, type = 2)
    public class HandlerDoSetPlayerBornDataNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.DoSetPlayerBornDataNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.DoSetPlayerBornDataNotifyOuterClass.DoSetPlayerBornDataNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.DoSetPlayerBornDataNotifyOuterClass.DoSetPlayerBornDataNotify.parseFrom(payload)
                            ), p);
