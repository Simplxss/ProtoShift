package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetPlayerBlacklistRsp, type = 2)
    public class HandlerGetPlayerBlacklistRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetPlayerBlacklistRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetPlayerBlacklistRspOuterClass.GetPlayerBlacklistRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetPlayerBlacklistRspOuterClass.GetPlayerBlacklistRsp.parseFrom(payload)
                            ), p);
