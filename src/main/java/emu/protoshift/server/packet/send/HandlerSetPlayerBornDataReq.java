package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.SetPlayerBornDataReq, type = 2)
    public class HandlerSetPlayerBornDataReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.SetPlayerBornDataReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.SetPlayerBornDataReqOuterClass.SetPlayerBornDataReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.SetPlayerBornDataReqOuterClass.SetPlayerBornDataReq.parseFrom(payload)
                            ), p);
