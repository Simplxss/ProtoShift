package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.PlayerChatCDNotify, type = 2)
    public class HandlerPlayerChatCDNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.PlayerChatCDNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.PlayerChatCDNotifyOuterClass.PlayerChatCDNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.PlayerChatCDNotifyOuterClass.PlayerChatCDNotify.parseFrom(payload)
                            ), p);
