package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EntityJumpNotify, type = 2)
    public class HandlerEntityJumpNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EntityJumpNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EntityJumpNotifyOuterClass.EntityJumpNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EntityJumpNotifyOuterClass.EntityJumpNotify.parseFrom(payload)
                            ), p);
