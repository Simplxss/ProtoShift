package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.BackMyWorldReq, type = 2)
    public class HandlerBackMyWorldReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.BackMyWorldReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.BackMyWorldReqOuterClass.BackMyWorldReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.BackMyWorldReqOuterClass.BackMyWorldReq.parseFrom(payload)
                            ), p);
