package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetOnlinePlayerListReq, type = 2)
    public class HandlerGetOnlinePlayerListReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetOnlinePlayerListReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetOnlinePlayerListReqOuterClass.GetOnlinePlayerListReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetOnlinePlayerListReqOuterClass.GetOnlinePlayerListReq.parseFrom(payload)
                            ), p);
