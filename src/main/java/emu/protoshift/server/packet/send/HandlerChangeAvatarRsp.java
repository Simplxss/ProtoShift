package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ChangeAvatarRsp, type = 2)
    public class HandlerChangeAvatarRsp extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ChangeAvatarRsp, 1), encryptType);
                var p = emu.protoshift.net.newproto.ChangeAvatarRspOuterClass.ChangeAvatarRsp.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ChangeAvatarRspOuterClass.ChangeAvatarRsp.parseFrom(payload)
                            ), p);
