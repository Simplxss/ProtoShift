package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.ScenePointUnlockNotify, type = 2)
    public class HandlerScenePointUnlockNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.ScenePointUnlockNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.ScenePointUnlockNotifyOuterClass.ScenePointUnlockNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.ScenePointUnlockNotifyOuterClass.ScenePointUnlockNotify.parseFrom(payload)
                            ), p);
