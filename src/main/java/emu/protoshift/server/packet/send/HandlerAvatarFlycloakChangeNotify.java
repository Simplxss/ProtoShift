package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.AvatarFlycloakChangeNotify, type = 2)
    public class HandlerAvatarFlycloakChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.AvatarFlycloakChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.AvatarFlycloakChangeNotifyOuterClass.AvatarFlycloakChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.AvatarFlycloakChangeNotifyOuterClass.AvatarFlycloakChangeNotify.parseFrom(payload)
                            ), p);
