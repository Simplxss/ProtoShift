    ...
    public class HandlerFishBiteReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.FishBiteReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.FishBiteReqOuterClass.FishBiteReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.FishBiteReqOuterClass.FishBiteReq.parseFrom(payload)
                            ), p);
