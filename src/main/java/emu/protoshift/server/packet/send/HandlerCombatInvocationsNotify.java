package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.CombatInvocationsNotify, type = 2)
    public class HandlerCombatInvocationsNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.CombatInvocationsNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.CombatInvocationsNotifyOuterClass.CombatInvocationsNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.CombatInvocationsNotifyOuterClass.CombatInvocationsNotify.parseFrom(payload)
                            ), p);
