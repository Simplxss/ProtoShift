package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.GetAllUnlockNameCardReq, type = 2)
    public class HandlerGetAllUnlockNameCardReq extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.GetAllUnlockNameCardReq, 1), encryptType);
                var p = emu.protoshift.net.newproto.GetAllUnlockNameCardReqOuterClass.GetAllUnlockNameCardReq.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.GetAllUnlockNameCardReqOuterClass.GetAllUnlockNameCardReq.parseFrom(payload)
                            ), p);
