package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.LifeStateChangeNotify, type = 2)
    public class HandlerLifeStateChangeNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.LifeStateChangeNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.LifeStateChangeNotifyOuterClass.LifeStateChangeNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.LifeStateChangeNotifyOuterClass.LifeStateChangeNotify.parseFrom(payload)
                            ), p);
