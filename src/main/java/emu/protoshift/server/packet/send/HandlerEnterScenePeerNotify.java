package emu.protoshift.server.packet.send;
    ...
    @Opcodes(value = PacketOpcodes.oldOpcodes.EnterScenePeerNotify, type = 2)
    public class HandlerEnterScenePeerNotify extends PacketHandler {
        public static class Packet extends BasePacket {
            public Packet(byte[] header, EncryptType encryptType, byte[] payload) {
                super(header, new PacketOpcodes(PacketOpcodes.newOpcodes.EnterScenePeerNotify, 1), encryptType);
                var p = emu.protoshift.net.newproto.EnterScenePeerNotifyOuterClass.EnterScenePeerNotify.newBuilder();
                try{
                    JsonFormat.parser().ignoringUnknownFields().merge(
                            JsonFormat.printer().printingEnumsAsInts().print(
                                    emu.protoshift.net.oldproto.EnterScenePeerNotifyOuterClass.EnterScenePeerNotify.parseFrom(payload)
                            ), p);
