...
class MT19937 internal constructor(seed: ULong) {
    ...
    companion object {
         ...
         */
        @JvmStatic
        fun generateKey(seed: Long): ByteArray {
            val gen = MT19937(MT19937(seed.toULong()).generate()).also { it.generate() }
        ...
        private fun ULong.toByteArray() = ByteArray(8).also { a ->
            val l = toLong()
            a[0] = l.shr(0).toByte()
            a[1] = l.shr(8).toByte()
            a[2] = l.shr(16).toByte()
